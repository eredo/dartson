...
var _DARTSON_ENTITY_NAME = mirrors.MirrorSystem.getName(mirrors.reflectClass(Entity).simpleName);
var _DARTSON_PROPERTY_NAME = mirrors.MirrorSystem.getName(mirrors.reflectClass(Property).simpleName);
var _DARTSON_STATIC_ENTITY = mirrors.MirrorSystem.getName(mirrors.reflectClass(StaticEntity).simpleName);
...
class FileCompiler extends _ErrorCollector {
  ...
  String get _staticEntityInterface => (_dartsonPrefix != null ? _dartsonPrefix.toString() + '.' : '') +
...
//    _rewriteCalls();
  ...
  void _findDartsonEntities() {
    entities.addAll(compilationUnit.declarations.where((m) => 
        m is ClassDeclaration && m.metadata.any((n) => _isDartsonAnnotation(n, _DARTSON_ENTITY_NAME))));
  ...
  void _prepareEntities() {
    entities.forEach((entity) {
      ...
      editor.editor.edit(entity.endToken.end - 1, entity.endToken.end - 1, '${encodeMethod}\n${decodeMethod}\n${newEntityMethod}\n');
    });
  }
  ...
  void _rewriteClassDeclarations() {
    entities.forEach((entity) {
      if (entity.implementsClause != null) {
        editor.editor.edit(entity.implementsClause.endToken.end, entity.implementsClause.endToken.end, ', ${_staticEntityInterface} ');
      } else if (entity.extendsClause != null) {
        editor.editor.edit(entity.extendsClause.endToken.end, entity.extendsClause.endToken.end,
  ...
  void _rewriteImports() {
    ImportDirective dir = compilationUnit.directives.firstWhere((directive) => directive is ImportDirective &&
        directive.uri.stringValue == 'package:dartson/dartson.dart', orElse: () => null);
    
    if (dir != null) {
      editor.editor.edit(dir.uri.beginToken.offset, dir.uri.endToken.end, '\'package:dartson/dartson_static.dart\'');
      _hasEdits = true;
    }
  }
  ...
  void _rewriteCalls() {
    var visitor = new DartsonMethodVisitor(_dartsonPrefix != null ? _dartsonPrefix.toString() : null);
  ...
  bool _isDartsonAnnotation(Annotation annotation, String className) =>
    (_dartsonPrefix != null ? 
        (annotation.name is PrefixedIdentifier && 
            (annotation.name as PrefixedIdentifier).prefix.toString() == _dartsonPrefix.toString() &&
                (annotation.name as PrefixedIdentifier).identifier.name == className) : 
  ...
  SimpleIdentifier findDartsonImportName() {
    ...
    var directive = compilationUnit.directives.firstWhere((directive) => directive is ImportDirective &&
        directive.uri.stringValue == 'package:dartson/dartson.dart', orElse: () => null);
    ...
    entity.members.forEach((ClassMember member) {
      if (member is FieldDeclaration) {
        ...
        if (member.fields.type.typeArguments != null) {
          member.fields.type.typeArguments.arguments.forEach((arg) => typeArguments.add(arg.name.name));          
        }
        ...
        member.fields.variables.forEach((VariableDeclaration d) {
          ...
          list.add(new PropertyDefinition(type, typeArguments, serializedName, d.name.name));
        });
      }
    });
  ...
  Property _findDartsonProperty(NodeList<Annotation> meta) {    
    Annotation annotation = meta.firstWhere((m) => 
        _isDartsonAnnotation(m, _DARTSON_PROPERTY_NAME), orElse: () => null);
    ...
    if (annotation != null) {
      ...
      return new Property(ignore: argsMap['ignore'] == true, name: argsMap['name']);
    } else {
      return null;
    }
  }
  ...
  String _buildNewEntityMethod(ClassDeclaration entity) => '${entity.name.toString()} newEntity() => new ${entity.name.toString()}();';
}
...
class PropertyDefinition {
  ...
  PropertyDefinition(this.type, this.typeArguments, this.serializedName, this.name);
  
  bool get isSimpleType => SIMPLE_TYPES.contains(type) && (typeArguments == null || typeArguments.isEmpty);
...
class _SimpleTransformWriter extends _TypeTransformWriter {
  ...
  @override
  String decode(String target, String object, [PropertyDefinition definition]) =>
    definition != null ? '${target}.${definition.name} = ${object}["${definition.serializedName}"];' :
      '${target} = ${object};';

  @override
  String encode(String target, String object, [PropertyDefinition definition]) =>
    definition != null ? '${object}["${definition.serializedName}"] = ${target}.${definition.name};' :
      '${object} = ${target};';
}
...
class _MapTransformWriter extends _TypeTransformWriter {
  ...
  @override
  String decode(String target, String object, [PropertyDefinition definition]) {
    var resp = ['if (${object}["${definition.serializedName}"] != null) {',
    ...
    if (SIMPLE_TYPES.contains(definition.typeArguments[1])) {
      resp.add(_simpleTransformer.decode('${target}.${definition.name}[keyVal]', 'val'));
    } else {
      resp.add(_entityTransformer.decode('${target}.${definition.name}[keyVal]', 'val',
        new PropertyDefinition(definition.typeArguments[1], null, null, null)));
    }
  ...
  @override
  String encode(String target, String object, [PropertyDefinition definition]) {
    if (definition == null) throw 'Unable to decode Map without arguments. Use SimpleTransformWriter instead.';
    
    var resp = ['if (${target}.${definition.name} != null) {',
      '  ${object}["${definition.serializedName}"] = {};',
      '  ${target}.${definition.name}.forEach((key, val) {'];
      
    if (SIMPLE_TYPES.contains(definition.typeArguments[1])) {
      resp.add(_simpleTransformer.encode('val', '${object}["${definition.serializedName}"][key]'));
    
      // TODO: Add nested generics support
    } else {
      resp.add(_entityTransformer.encode('val', '${object}["${definition.serializedName}"][key]',
                                         new PropertyDefinition(definition.typeArguments[1], null, null, null)));
    }
...
class _ListTransformWriter extends _TypeTransformWriter {
  ...
  @override
  String decode(String target, String object, [PropertyDefinition definition]) {
    if (definition == null) throw 'Unable to decode List without arguments. Use SimpleTransformWriter instead.';
    ...
    if (SIMPLE_TYPES.contains(definition.typeArguments[0])) {
      ...
      resp.add(_entityTransformer.decode('el', 'val',
        new PropertyDefinition(definition.typeArguments[0], null, null, null)));
    }
  ...
  @override
  String encode(String target, String object, [PropertyDefinition definition]) {
    if (definition == null) throw 'Unable to encode List without arguments. Use SimpleTransformWriter instead.';
    ...
    if (SIMPLE_TYPES.contains(definition.typeArguments[0])) {
      ...
      resp.add(_entityTransformer.encode('val', 'el',
                                         new PropertyDefinition(definition.typeArguments[0], null, null, null)));
    }
